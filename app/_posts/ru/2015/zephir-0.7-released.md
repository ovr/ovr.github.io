...
Почему такой выигрыш? Давайте разбираться. Обычный PHP land метод сгенерированный Zephir будет выглядить таким образом:
```c
 ...
 * this_ptr это zend_class_entry на текущий класс
 *
 * Внимание! Параметры передаются указателями созданными на основе stack переменных
 * stack &a -> (zval *a) 
 */
...
* Нет получения параметров из стека.
